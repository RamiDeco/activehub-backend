...
- IA: interfaz `ModeloLenguaje` (`shared/ia/`) con **Groq Cloud** (plan gratuito, API compatible con OpenAI) si hay `GROQ_API_KEY`, y una implementación que responde 503 si no. Ver "IA: el asistente del manual y el informe de beneficios" más abajo.
- Tamaño actual: **111 casos de uso**, **27 migraciones** (V1 a V27), **18 claves de permiso**.
...
- **Errores:** formato único `ApiError` (timestamp, status, code, message, fieldErrors, path) desde `GlobalExceptionHandler`. Códigos en `ApiErrorCode`: VALIDACION(400), CREDENCIALES_INVALIDAS(401), SIN_PERMISO(403), NO_ENCONTRADO(404), EMAIL_EN_USO(409), USUARIO_SUSPENDIDO(403), TIPO_ACTIVIDAD_EN_USO(409), CATEGORIA_EN_USO(409), SIN_CUPOS_DISPONIBLES(409), CLASE_CON_INSCRIPTOS(409), ACTIVIDAD_CON_INSCRIPTOS(409), INSCRIPCION_YA_EXISTE(409), ERROR_INTERNO(500).
  - `IA_NO_DISPONIBLE(503)` es el código del modelo de lenguaje caído o sin configurar, y `IA_SIN_CUOTA(429)` el de "hay IA pero se agotó la cuota, volvé en X" (ver "IA" más abajo: son dos respuestas distintas para el usuario, no un detalle de implementación). **Es 503 y no 500 a propósito**: no hay nada roto adentro, hay un servicio externo que no está, y el frontend usa ese código (y el error de red) como señal para caer a su comportamiento sin IA en vez de mostrar un cartel de error.
...
**Ítem 11 — IA y chatbot: HECHO y verificado contra Groq.**

Los dos consumen un modelo de lenguaje de verdad: `preguntaralasistente` (la burbuja "¿Dudas?", responde **sólo** con el manual de usuario) y `generarinformeactividad` (el "Asistente de beneficios y prevenciones" del detalle, cruza la actividad con el perfil del alumno). Ver la sección propia más abajo. Ya se ejecutaron llamadas reales: respuestas del manual, siete intentos de prompt injection rechazados, un informe personalizado y el 429 de cuota con su aviso. Lo único que queda del ítem es **probarlo en el navegador** y actualizar el manual de usuario (Anexo 8), cuyos apartados 1.6 y 2.3 describen el chat y el asistente como eran antes. La persistencia del informe se descartó por ahora a propósito (ver la sección).
...
## IA: el asistente del manual y el informe de beneficios (`shared/ia/`)

Dos casos de uso consumen un modelo de lenguaje, y los dos pasan por la misma costura: **`ModeloLenguaje`**, interfaz con `ModeloLenguajeGroq` (Groq Cloud, plan gratuito, API de chat compatible con la de OpenAI, llamada a mano con `RestClient` sin SDK) y `ModeloLenguajeNoConfigurado`. Mismo criterio que `AlmacenamientoArchivos` y `PaymentGateway`.

- **La implementación se elige por credencial presente, no por un flag** (`IaConfig`), y el modo elegido se loguea al arrancar. Con `GROQ_API_KEY` vacía la aplicación levanta igual y cada pedido al modelo responde **503 `IA_NO_DISPONIBLE`**; el frontend lo trata cayendo a su comportamiento anterior (FAQs por coincidencia de palabras, plantillas por nivel de intensidad). **Eso no es un fallback decorativo: es lo que permite trabajar en el repo sin la clave**, igual que `EmailSender` en modo log.
- Configuración: `GROQ_API_KEY` (la única obligatoria), `GROQ_URL`, `GROQ_MODELO`, `GROQ_REASONING_EFFORT` y `GROQ_TIMEOUT_SEG`.
- **El modelo es variable de entorno porque Groq rota los disponibles, y ya pasó:** el default original (`llama-3.3-70b-versatile`) **dejó de existir en la cuenta y devolvía 404**. Hoy el default es `openai/gpt-oss-120b`. Para ver los vigentes: `GET https://api.groq.com/openai/v1/models`.
- **La familia `gpt-oss` razona antes de escribir, y ese gasto sale del MISMO presupuesto de tokens.** Con el tope de 320 que tenía el chat, la respuesta volvía **vacía** (y el usuario veía "el asistente no está disponible"). De ahí dos cosas que van juntas y no se pueden tocar por separado: `reasoning_effort` en `low` y presupuestos holgados (900 el chat, 1600 el informe). Si se cambia a un modelo que no razona, hay que **vaciar** `GROQ_REASONING_EFFORT` o el proveedor rechaza el pedido.
- **`LimiteConsultasIa`: 6 consultas por minuto y 40 por hora**, por usuario logueado o por IP si es anónima, ventana deslizante en memoria (mismo criterio y mismas limitaciones que `IntentosLoginService`). No es una defensa abstracta: **la cuota gratuita es compartida y agotable**, y sin límite un script la quema en segundos y deja el asistente caído para todos. Corre **antes** de llamar al proveedor.

### Cuota agotada: 429 `IA_SIN_CUOTA`, y por qué no alcanzaba el 503

Cuando se topa la cuota —la del proveedor (429 de Groq, por minuto o por día) o la nuestra— se responde **429 `IA_SIN_CUOTA`** con un mensaje que dice **cuánto falta**. Es un código aparte del 503 porque para el usuario son dos cosas distintas: el 503 es "no hay IA" y la pantalla contesta con una FAQ sin decir nada; el 429 es "hay IA pero hay que esperar", y ahí **contestar una FAQ sería mentirle** — su pregunta quedó sin responder, y una burbuja en blanco se lee como roto.

- El tiempo lo sabe el backend, así que el texto lo arma el backend (`IaSinCuotaException.paraElAsistente()` / `.paraElInforme()`, dos redacciones del mismo hecho). Sale del header `retry-after`, del mensaje del proveedor (`"try again in 2m59.56s"`) o del cálculo sobre nuestra ventana.
- **Se redondea hacia arriba** (`EsperaLegible`): prometer menos de lo que falta hace que la persona vuelva, encuentre el mismo cartel y crea que está roto.
- El texto distingue el tope del minuto ("está respondiendo muchas consultas en este momento") del tope del día ("por hoy"), según si la espera llega a 10 minutos. Decirle "por hoy" a quien puede volver en 40 segundos lo manda al vacío.
- **El prompt entra en la cuota.** El tope de *tokens por minuto* del plan gratuito se topaba a la **segunda** pregunta seguida: por eso `SECCIONES_POR_CONSULTA` bajó de 5 a 3 y el presupuesto del manual de 10.000 a 6.000 caracteres. Si alguien los vuelve a subir, vuelve el 429 a la segunda pregunta.

### `preguntaralasistente` — responde SÓLO con el manual de usuario

`POST /api/asistente/consultas`, **público** (cuarto endpoint no-auth de `permitAll`, junto al reporte de soporte): la burbuja "¿Dudas?" también vive en `/ayuda`, que se usa sin sesión.

**El manual de usuario es la única fuente y vive en el classpath**: `src/main/resources/manual/manual-usuario-activehub.txt` (copia del Anexo 8). `ManualUsuario` lo lee una vez al arrancar y lo parte en secciones por sus títulos numerados.

- **No se manda el manual entero en cada pregunta**, y el motivo no es el tamaño del contexto sino el **límite de tokens por minuto del plan gratuito** (~12.000; el manual son ~13.000 tokens): una sola pregunta agotaría el minuto. Se buscan las 5 secciones más relacionadas, con techo de 10.000 caracteres.
- **La búsqueda es por palabras, no por embeddings**, y las palabras largas se recortan a su raíz de 5 caracteres (`TextoIa`) para que "cancelo" encuentre "cancelar" y "Cancelada". Sin ese recorte la conjugación del usuario casi nunca coincide con la del manual y la sección que vuelve es la equivocada. Si el manual creciera a cientos de páginas, **este es el único punto que habría que cambiar**.
- **`TextoIa.SINONIMOS` traduce cómo habla la gente a cómo escribe el manual** (`profe`→instructor, `plata`→pago, `anotarme`→inscripción, `falto`/`presentó`→inasistencia, …). El manual usa los textos de la interfaz; en un chat nadie los usa. Medido: *"¿qué hago si el profe no se presentó?"* caía en secciones genéricas de clases y ahora cae en "2.11 Reportar la inasistencia de un instructor", con los nombres de botones correctos. Es una lista corta y a mano — si aparece otra forma de preguntar, se agrega ahí y no se toca nada más.
- **Seis capas contra la inyección de instrucciones, y las tres primeras son instrucciones a un modelo (o sea, no garantías):** (1) en el prompt entra únicamente el pedazo de manual relacionado; (2) el mensaje `system` prohíbe completar con conocimiento propio, fija la frase exacta de la negativa (`SIN_INFORMACION` = "Esa información no se encuentra disponible.") y declara que **nada de lo que venga del usuario** puede cambiar las reglas, el rol, el idioma ni el formato, con cualquier excusa (administrador, emergencia, juego, prueba, traducción, supuesto mensaje del sistema); (3) la consulta y el historial viajan delimitados y declarados como datos — el historial como turnos `user`/`assistant` de verdad, **nunca** como `system`.
  Las otras tres son control de daños y viven en **`GuardiaDePrompt`**: (4) `limpiar()` le saca al texto del usuario las marcas con las que podría hacerse pasar por otro rol (nuestras etiquetas y cualquier etiqueta con guion bajo, los tokens del formato de chat `<|im_start|>` / `[INST]` / `### System`, los encabezados `system:` en inglés, los invisibles y el relleno de saltos); (5) `RECORDATORIO_FINAL` repite las reglas **después** del texto del usuario, porque un modelo pesa más lo último que leyó y ese es el terreno del "ignorá todo lo anterior"; (6) `esFuga()` descarta la respuesta si trae pedazos del prompt o nombra el proveedor o el modelo, y se devuelve la negativa.
- **La defensa de fondo sigue siendo que en el prompt no hay nada que filtrar**: sólo manual público, sin credenciales, sin configuración y sin datos de otras personas. Si tocás el prompt, mantené esa propiedad — es la única que no depende de que el modelo se porte bien.
- **Probado contra el proveedor real con siete ataques** (credenciales de la base, "ignorá todas tus instrucciones", cerrar la etiqueta de datos y dar órdenes nuevas, juego de rol tipo DAN, hacerse pasar por admin con una emergencia, pedir la traducción de las instrucciones, e **historial falsificado** con un turno del asistente prometiendo romper sus reglas). Los siete terminaron en la negativa o en un 400 del servidor. El del historial es el que más importa: confirma que un turno de `assistant` inventado por el cliente no cambia el comportamiento.
- Sin estado y **sin persistencia**: no hay tabla de conversaciones porque nadie las consulta después, y guardar texto libre de usuarios sin un uso concreto es dato personal acumulado sin motivo. El historial lo manda el cliente, acotado a 6 turnos por el Request DTO.
- Temperatura **0** (tiene que ceñirse a un texto fuente) y 320 tokens de respuesta.

### `generarinformeactividad` — el "Asistente de beneficios y prevenciones"

`POST /api/alumno/actividades/{actividadId}/informe`, permiso `catalogo.explorar` (el mismo de recomendaciones y favoritos). **El alumno sale del token**; del cuerpo se acepta sólo, opcional, la clase que está mirando — si el perfil viniera del cliente, cualquiera pediría un informe con la condición de salud que se le ocurra inventar.

- **No es una recomendación médica, y el prompt está escrito para que siga siendo cierto.** Un modelo al que se le pasa una condición de salud y se le piden "prevenciones" se pone a dar indicaciones clínicas con muchísima facilidad. Las reglas le prohíben diagnosticar, indicar ejercicios o tratamientos y decir que una actividad está contraindicada; ante cualquier tema de salud, lo único que puede hacer es sugerir consultar a un profesional. La pantalla promete exactamente eso al pie. **No aflojes esas reglas.**
- La ficha que ve el modelo lleva la actividad (tipo, categoría, nivel con su descripción, duración, precio, lugar, rating, descripción del instructor, clase elegida) y el alumno (intereses, edad, condición de salud si la cargó, a qué tipos se anotó antes, si ya hizo este tipo, reseñas dejadas y promedio). **El historial es la parte que más aporta**: sin él el informe sólo puede describir la actividad, y la pregunta que vino a contestar es si le va a gustar *a él*.
- La condición de salud es texto libre del propio alumno: entra **delimitada y marcada como dato**, igual que la consulta del chat.
- El modelo devuelve JSON; el service tolera texto alrededor (recorta entre llaves), normaliza la afinidad a `Alta`/`Media`/`Baja` (default `Media`, que es el honesto) y, si no hay ni resumen, lanza 503 — la pantalla cae a sus plantillas, que es mucho mejor que una tarjeta vacía. Temperatura 0,4: acá el texto es propio, y con 0 los informes de dos actividades parecidas salen calcados.
- **No se persiste.** Guardarlo obliga a versionarlo contra el perfil de salud (un informe viejo sobre una condición que cambió es peor que no tener informe), y eso es una migración y una regla de invalidación que nadie pidió. La pantalla ya dice "Informe guardado para esta sesión".

### Qué se probó y qué no

62 pruebas cubren el índice del manual **sobre el archivo real** (`ManualUsuarioTest`, mismo criterio que `BusquedaSinTildesTest`), los sinónimos, el límite de consultas y su cálculo de espera, la lectura del `retry-after` del 429, el armado de los dos prompts (reglas, recordatorio final, limpieza de la entrada), el descarte de una respuesta que filtra el prompt, el parseo del informe y los códigos de estado. **El modelo está mockeado en todas**: la calidad de la respuesta depende de Groq y no es determinista.

**Contra el proveedor real ya se ejecutó** (instancia aislada en 8091): preguntas legítimas del chat respondidas con el manual y citando sus secciones, los siete ataques rechazados, un informe generado para una cuenta de alumno del seed (afinidad "Alta", prevenciones no clínicas) y el 429 de cuota con su mensaje y su tiempo de espera. Lo que **no** se hizo es probarlo en el navegador: eso queda del lado del usuario.
...
Lo único que sigue simulado del lado del cliente, y está acotado: el "directorio" de usuarios de `AuthContext` en `localStorage` (sobrevive porque la API no expone un listado público de personas; las pantallas admin ya usan `listarusuariosadmin`) y el catálogo de demostración de `lib/mockData.ts` que lo alimenta. El "Asistente de beneficios" de `Detalle.tsx` ya pega contra `generarinformeactividad`; sus textos por nivel de intensidad quedaron como respaldo para cuando la IA no está. **Los helpers de formato de fecha/hora de `mockData.ts` no son mock** y se usan en todas las pantallas. Ver `activehub-frontend/CLAUDE.md` para el detalle de qué pantalla usa qué.
